...
from src.config import AUDIO_DIR, ROOT, HOST, PORT, SSL_KEYFILE, SSL_CERTFILE
...
# ------------------------------------------------------------------
# 全局服务实例（单例）。模型权重在 lifespan 启动阶段预加载，
# 避免首次请求等待；同时不阻塞 event loop。
# ------------------------------------------------------------------
...
async def lifespan(_app: FastAPI):
    # ---- startup ----
    await asyncio.to_thread(_sign_model.load)
    await asyncio.to_thread(translate_service.warmup)
    yield
    # ---- shutdown ----
    _sign_model.unload()
